...
contract OracleDistributor is Ownable {
    ...
    function LPConvert() external onlyEOA onlyHolder {
        ...
        while (gasUsed < limit_gas && iterations < len) {
            IOracleSwapPair pair = IOracleSwapPair(factory.allPairs(iterations));
    ...
    function LPEnalbe() external view returns (bool) {
        ...
        for (uint256 i = 0; i < len; i++) {
            IOracleSwapPair pair = IOracleSwapPair(factory.allPairs(i));
            uint256 lpBalance = pair.balanceOf(address(this));
            if (lpBalance > MIN_LP_AMOUNT) {
                if (onlyOracleLp) {
                    if (
    ...
    function _swap(
        ...
        // Checks
        IOracleSwapPair pair = IOracleSwapPair(
