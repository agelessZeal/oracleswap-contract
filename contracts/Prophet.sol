...
contract ProphetToken is IERC20, Ownable {
    ...
    modifier onlyOwner() {
        require(
            msg.sender == ownerMasterProphet ||
                msg.sender == ownerNftStaking ||
                msg.sender == ownerFTSO,
            "User is not owner"
        );
        _;
    }
    ...
    function setOwner(
        address ownerMasterProphet_,
        address ownerNftStaking_,
        address ownerFTSO_
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "ORACLE::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "ORACLE::delegateBySig: invalid nonce"
        );
        require(block.timestamp <= expiry, "ORACLE::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "ORACLE::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "ORACLE::_writeCheckpoint: block number exceeds 32 bits"
        );
